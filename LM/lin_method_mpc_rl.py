...
class MHOQ_RLIM:
    ...
    def q_scaling(self, X):
        ...
        """
        # return np.floor(X.squeeze() / self.Qstep + 2**(self.Nb - 1))   # Normalize based on bit depth
        return X.squeeze() / self.Qstep + 2**(self.Nb - 1)
    ...
    def get_codes(self, N_PRED, Xcs, YQns, MLns, Step):
        ...
        for j in tqdm.tqdm(range(len_MPC)):
            ...
            u = m.addMVar((2**self.Nb, N_PRED), vtype=GRB.BINARY, name="u")  # Control variables (binary choice)
           ...
           # Loop through prediction horizon to set constraints and objective
            for i in range(N_PRED):
                ...
                bin_con = QL_I.reshape(1, -1) @ u[:, i].reshape(-1, 1)  # Convert binary input to actual control value
                ...
                # Add e_step to alow headroom when finding a solution between larger xref steps
                e_step.append(abs(u_kminus1_ind[i] - Xcs[j + i]))

                # Define search bounds for control input
                ub = min(int(u_kminus1_ind[i] + Step + e_step[j]), 2**self.Nb - 1)
                lb = max(int(u_kminus1_ind[i] - Step - e_step[j]), 0)
            ...
            u_val = values[:u.size].reshape(2**self.Nb, N_PRED).astype(int)

            # Extract optimal DAC code
            C_MPC = np.array([np.nonzero(u_val[:, i])[0][0] for i in range(N_PRED)])
